...
NOTIFICATION_TEMPLATES = {
    ...
    "new_order": (_noop("Novo Pedido, O'Atelier"), _noop(
        """Novo pedido para o usuário {client_name} 
        por BRL: {price_brl}$ USD: {price_usd}$""")),

    "comm_cancelation_client": (_noop("Pedido Cancelado, O'Atelier"), _noop(
    ...
    "comm_accepted": (_noop("Pedido Aceito, O'Atelier"), _noop(
    ...
    "comm_update": (_noop("Atualização de Status do Pedido, O'Atelier"), _noop(
        """O status do pedido: {uuid}, foi atualizado. 
        {previous_stage} -> {current_stage}. 
        """)),

    "comm_update_final": (_noop("Processo de Produção do Pedido Concluído, O'Atelier"), _noop(
        """O status do pedido: {uuid}, foi atualizado. 
        {previous_stage} -> {current_stage}.
        Por favor, realize o pagamento do valor restante do pedido para prosseguir.
        """)),

    "deposit_confirmation_artist": (_noop("Pagamento da Garantia Recebido, O'Atelier"), _noop(
        """O pagamento da garantia do pedido: {uuid} foi realizada. 
        O pedido pode começar a ser produzido.
        """)),

    "deposit_confirmation": (_noop("Pagamento Confirmado, O'Atelier"), _noop(
    ...
    "full_confirmation_artist": (_noop("Pagamento Final Recebido, O'Atelier"), _noop(
        """O pagamento final do pedido: {uuid} foi realizado. 
        O pedido foi finalizado e a arte final já está disponível para o cliente.
        """)),

    "full_confirmation": (_noop("Pedido Concluído, O'Atelier"), _noop(
...
def render_notification(notification: Notification) -> dict[str, str | bool | datetime | Any]:
    ...
    message = template[1].format(**notification.context)
    notification_data = {
        "pk": notification.pk,
        "message": message,
